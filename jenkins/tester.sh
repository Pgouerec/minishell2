...
load_test()
{
  ...
  then
    ...
    if [ $debug -ge 1 ]
    then
      echo "Test $id ($NAME) : KO - Check output in /tmp/minishell2/$id/" 
      # $MKDIR -p /tmp/test.$$/$id 2>/dev/null
      $MKDIR -p /tmp/minishell2/$id 2>/dev/null
      # $CP /tmp/.shell.$$ /tmp/test.$$/$id/mysh.out
      # $CP /tmp/.refer.$$ /tmp/test.$$/$id/tcsh.out
      $CP /tmp/.shell.$$ /tmp/minishell2/$id/mysh.out
      $CP /tmp/.refer.$$ /tmp/minishell2/$id/tcsh.out
    else
      echo "KO"
    fi
  fi
